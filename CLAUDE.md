...
| `BATON_BD_BACKEND` | Bead-store backend (ADR-13b). `sqlite` = legacy SQLite store; `bd` = the external `bd` CLI (gastownhall/beads) as system of record; `auto` (default) = use `bd` when enabled and present (the installer puts it on PATH), else fall back to SQLite. | `auto` |
