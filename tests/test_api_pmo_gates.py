...
class TestApprovalExceptionMapping:
    ...
    # ------------------------------------------------------------------
    # InvalidApprovalState — approve endpoint
    # ------------------------------------------------------------------
    ...
    def test_not_approval_pending_approve_returns_409(
        ...
        with patch(
            "agent_baton.core.engine.executor.ExecutionEngine.record_approval_result",
            side_effect=InvalidApprovalState(reason="not_approval_pending"),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
                json={"phase_id": 1},
    ...
    def test_no_approval_requested_approve_returns_409(
        ...
        with patch(
            "agent_baton.core.engine.executor.ExecutionEngine.record_approval_result",
            side_effect=InvalidApprovalState(reason="no_approval_requested"),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
                json={"phase_id": 1},
    ...
    # ------------------------------------------------------------------
    # InvalidApprovalState — reject endpoint
    # ------------------------------------------------------------------
    ...
    def test_compliance_write_error_approve_returns_503(
        ...
        with patch(
            ...
            side_effect=ComplianceWriteError(
                "Audit write failed", audit_path="/var/audit/log"
            ),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
        ...
        assert body["details"]["audit_path"] == "/var/audit/log"
    ...
    def test_compliance_write_error_reject_returns_503(
        ...
        with patch(
            "agent_baton.core.engine.executor.ExecutionEngine.record_approval_result",
            side_effect=ComplianceWriteError("Audit write failed"),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/reject",
                json={"phase_id": 1, "reason": "Bad output"},
    ...
    def test_execution_state_inconsistency_approve_returns_500(
        ...
        with patch(
            ...
            side_effect=ExecutionStateInconsistency(
                ...
                operation="approve-with-feedback",
            ),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
        ...
        assert body["details"]["operation"] == "approve-with-feedback"
    ...
    def test_execution_state_inconsistency_reject_returns_500(
        ...
        with patch(
            "agent_baton.core.engine.executor.ExecutionEngine.record_approval_result",
            side_effect=ExecutionStateInconsistency("State reload failed"),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/reject",
                json={"phase_id": 1, "reason": "Bad output"},
    ...
    # ------------------------------------------------------------------
    # Body shape invariants for all error responses
    # ------------------------------------------------------------------
    ...
    def test_error_response_always_has_required_fields(
        ...
        with patch(
            "agent_baton.core.engine.executor.ExecutionEngine.record_approval_result",
            side_effect=InvalidApprovalState(reason="phase_mismatch", phase_id=5),
        ):
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
                json={"phase_id": 5},
    ...
    # ------------------------------------------------------------------
    # Regression: existing happy-path still works
    # ------------------------------------------------------------------

    def test_happy_path_approve_still_works(
        self, tmp_path: Path, registered_store: PmoStore
    ) -> None:
        """Ensure the new exception handling does not break normal approval flow."""
        card = _awaiting_card()
        client = _make_app(tmp_path, registered_store, [card])
        ...
        with patch(
            ...
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/approve",
                json={"phase_id": 1, "notes": "All good"},
            )
    ...
    def test_happy_path_reject_still_works(
        self, tmp_path: Path, registered_store: PmoStore
    ) -> None:
        """Ensure the new exception handling does not break normal rejection flow."""
        card = _awaiting_card()
        client = _make_app(tmp_path, registered_store, [card])
        ...
        with patch(
            ...
            r = client.post(
                f"/api/v1/pmo/gates/{card.card_id}/reject",
                json={"phase_id": 1, "reason": "Tests failing"},
            )
