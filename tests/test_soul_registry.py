...
Wave 6.1 Part B (bd-d975) + v33 revocation extension.
...
# ---------------------------------------------------------------------------
# SoulRegistry.revoke — v33 table-based
# ---------------------------------------------------------------------------
...
class TestSoulRegistryRevoke:
    ...
    def test_is_revoked_legacy_notes_fallback(self, registry: SoulRegistry):
        """is_revoked() falls back to notes-prefix for pre-v33 revocations."""
        soul = registry.mint("code-reviewer", "auth")
