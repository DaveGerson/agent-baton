...
# ---------------------------------------------------------------------------
# Test 1 — R3.8 migration applies cleanly on fresh DB
# ---------------------------------------------------------------------------

def test_migration_r38_applies_cleanly() -> None:
    """R3.8 migration DDL creates deployment_profiles table.

    Renumbered from v16 to v26 during merge into main (which already had
    v16 in use for tenancy/cost-attribution columns).
    """
    from agent_baton.core.storage.schema import MIGRATIONS, SCHEMA_VERSION

    assert 26 in MIGRATIONS, "MIGRATIONS[26] (R3.8) must exist"
    assert SCHEMA_VERSION >= 26

    conn = sqlite3.connect(":memory:")
    # v26 ALTERs releases — give it a stub releases table first so the
    # ALTER doesn't error.
    conn.executescript("""
    ...
    """)
    conn.executescript(MIGRATIONS[26])
