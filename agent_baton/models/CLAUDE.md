...
- `execution.py` — `ActionType` enum (9 values: `DISPATCH`, `GATE`, `APPROVAL`, `COMPLETE`, `FAILED`, `WAIT`, `FEEDBACK`, `INTERACT`, `CHECKPOINT`) and `ExecutionState`. **Adding a new `ActionType` is a protocol change** — coordinate with `agents/orchestrator.md`, `references/baton-engine.md`, and `cli/commands/execution/execute.py::_print_action`.
