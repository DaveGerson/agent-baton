"""``baton verify-package`` -- DEPRECATED shim.

Use ``baton install verify`` instead.
...
def register(subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    p = subparsers.add_parser(
        "verify-package",
        help="[DEPRECATED] Use 'baton install verify' instead",
        description=(
            "DEPRECATED: Use 'baton install verify' instead.\n\n"
...
def handler(args: argparse.Namespace) -> None:
    print(
        "WARN: 'baton verify-package' is deprecated, use 'baton install verify'",
        file=sys.stderr,
    )
