...
class ClaudeCodeLauncher:
    ...
    def _parse_output(
        ...
        if parsed is not None:
            ...
            redacted_text = _redact_sensitive(result_text)
            outcome = redacted_text[: self._config.max_outcome_length]
            if len(redacted_text) > self._config.max_outcome_length:
                self._warn_truncation(
                    agent_name=agent_name,
                    step_id=step_id,
                    bytes_attempted=len(redacted_text),
        ...
        outcome = redacted_raw[: self._config.max_outcome_length]
        if len(redacted_raw) > self._config.max_outcome_length:
            self._warn_truncation(
                agent_name=agent_name,
                step_id=step_id,
