...
agent_baton/core/engine/manager_scope_signal.py NEW SCOPE_EXPANSION: line parser (M9, models on gate_addition.py)
                                              NOTE: core/engine/scope_expansion.py ALREADY EXISTS (unrelated pre-existing
                                              feature: expansion phases from bead signals). Do NOT overwrite it. Inspect it
                                              first; reuse its parser only if the API genuinely fits, else build the new module.
...
**Files:** Modify `agent_baton/core/engine/executor.py` (three sites only: `_dispatch_action` ~L6600-6699, `_synthesize_beads_post_phase` L960-1048, `complete()` L4004), create `agent_baton/core/engine/manager_scope_signal.py`, `tests/e2e/test_manager_mode_execution_dry_run.py`, `tests/engine/test_manager_scope_signal.py`. WARNING: `core/engine/scope_expansion.py` + `tests/engine/test_scope_expansion.py` already exist (unrelated pre-existing feature) — read them first, do not overwrite; reuse only if the API fits.
...
- [ ] **Step 3: Implement.** All hooks fire only when `plan.manager_mode` and sidecars exist; every hook wrapped best-effort (`except Exception: logger.debug(...)`) EXCEPT scope-expansion `block` policy which must fail the step visibly. `manager_scope_signal.py`: `parse_scope_expansion_signals(text) -> list[ScopeExpansionSignal]` — regex `^SCOPE_EXPANSION:\s*(?P<path>\S+)\s*[—-]\s*(?P<reason>.+)$` multiline, mirror `gate_addition.py` structure (distinct from the pre-existing `scope_expansion.py` module — see Files warning). Dispatch: load `scope_contract(step_id, "md")` text and render bundle JSON into a compact `## Context Bundle` section (must_read/reference_only/pack names+paths — not full doc bodies). Post-phase: build handoff md from phase step_results (files_changed, summaries), decision beads, gap records; write via ManagerArtifactPaths; then `ManagerReportBuilder.save_report`. Also add the `SCOPE_EXPANSION` signal line to the dispatcher `_SIGNALS_BLOCK` (additive sentence documenting the format — verify existing block wording first). `complete()`: final report after retrospective.
