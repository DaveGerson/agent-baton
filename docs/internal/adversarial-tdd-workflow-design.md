...
| 5 | `implementation` | Implementation | *base plan's harvested implement steps* | `sonnet` | (preserved) | The steps the pipeline planned, flattened into one phase in original order (agents, descriptions, deps re-keyed, all other fields preserved), re-tiered to sonnet. Gate: the first test-or-build gate from the base plan (fallback: stack-derived gate). |
